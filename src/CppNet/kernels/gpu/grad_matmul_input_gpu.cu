...
__global__
void grad_input_kernel(const float* dY, const float* W, float* dX,
...
void matmul_grad_input_gpu(const float* dY, const float* W, float* dX,
                           int batch, int in_size, int out_size)
{
    float *ddY, *dW, *ddX;
    ...
    grad_input_kernel<<<grid, block>>>(ddY, dW, ddX,
